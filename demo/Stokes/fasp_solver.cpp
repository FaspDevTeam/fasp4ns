...
int FaspSolver::solve (EigenVector &x,EigenVector &b,bool solver_type, const int output)
{
    ...
    if (solver_type) {
        ...
        //%%%%%%%%%%% fasp-ns solve  %%%%%%%%%%%%%%%
        int flag = fasp_solver_dblc_krylov_navier_stokes (&blkFS, &blk_b_FS, &blk_u_FS, &itparam, &amgparam, &iluparam, &schparam);
        //recover solution
        ...
        int flag = fasp_solver_dblc_krylov_navier_stokes_schur_complement_with_pressure_mass (&blkFS, &blk_b_FS, &blk_u_FS, &itparam, &amgparam, &iluparam, &schparam,&M_P);
        //recover solution
        for (int i=0;i<gidx_u.size ();i++) {
            u_FS.val[gidx_u[i]] = blk_u_FS.val[i];
        }
