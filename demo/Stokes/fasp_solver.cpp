...
FaspSolver::FaspSolver() {
    FS.row = FS.col = FS.nnz = 0;
    FS.IA = FS.JA = NULL; FS.val = NULL;
    
    b_FS.row = 0; b_FS.val = NULL;
    u_FS.row = 0; u_FS.val = NULL;
    blk_b_FS.row = 0; blk_b_FS.val = NULL;
    blk_u_FS.row = 0; blk_u_FS.val = NULL;
    
    blkFS.brow = blkFS.bcol = 0;
    blkFS.blocks = NULL;
 ...
 * solve linear system
 */
int FaspSolver::solve (EigenVector &x,
                       EigenVector &b,
                       bool solver_type,
                       const int output)
{
    if (b.size() > b_FS.row) {
        if (b_FS.val!=NULL&&b_FS.row>0) fasp_mem_free (b_FS.val);
        b_FS.row = b.size();
    ...
    for (int i=0;i<b_FS.row;i++) b_FS.val[i] = bval[i];
    
    assemble_blk();
    
    if (output) { // output data files for debug purpose
    ...
    if (solver_type) {
        ///////// fasp-ns solve /////////
        int flag = fasp_solver_dblc_krylov_navier_stokes (&blkFS, &blk_b_FS,
                                                          &blk_u_FS, &itparam,
                                                          &amgparam, &iluparam,
                                                          &swzparam);
        
        // save solution
        for (int i=0;i<gidx_u.size();i++) u_FS.val[gidx_u[i]] = blk_u_FS.val[i];
        for (int i=0;i<gidx_p.size();i++) u_FS.val[gidx_p[i]] = blk_u_FS.val[n_v+i];
        
    }
    else {
        ///////// fasp-ns solve with pressure /////////
        int flag = fasp_solver_dblc_krylov_navier_stokes_schur_complement_with_pressure_mass (&blkFS, &blk_b_FS, &blk_u_FS, &itparam, &amgparam, &iluparam, &swzparam,&M_P);
    ...
    fasp_solver_umfpack (ptrA,&b_FS,&u_FS,5);
    fasp_dcsr_free (ptrA);
#endif
    ...
    for (int i = 0; i < n_FS; i++) xval[i] = u_FS.val[i];
...
void FaspSolver::assign_idx (FunctionSpace * W)
{
    ...
    W->dofmap()->extract_sub_dofmap (component, *(W->mesh()));
    gidx_u.clear();
    gidx_p.clear();
...
void FaspSolver::set_pressure_mass (EigenMatrix &P)
{
    ...
    for (i = 0; i< row+1; i++) IA[i] = IA_tmp[i];
    const int*JA_tmp = std::get<1> (tp);
    for (i = 0; i< nnz;i++) JA[i] = JA_tmp[i];
    
    double *val_tmp =  (double *)std::get<2> (tp);
    for (i = 0; i< nnz;i++) val[i] = val_tmp[i];
    n_p = gidx_p.size();
    Ip  = (INT*)fasp_mem_calloc (n_p,sizeof (INT));
    
    for (int i=0;i<gidx_p.size();i++) Ip[i] = gidx_p[i];
...
int FaspSolver::set_operator (EigenMatrix &A)
{
    ...
    FS.col = col;
    if (row>FS.row) {
        if (FS.IA!=NULL) fasp_mem_free (FS.IA);
    ...
    // Assign nnz
    if (nnz>FS.nnz) {
        if ( (FS.JA!=NULL)  && (FS.row>0) ) fasp_mem_free (FS.JA);
        if ( (FS.val!=NULL) && (FS.row>0) ) fasp_mem_free (FS.val);
...
int FaspSolver::assemble_blk()
{
    ...
    int count=0, count_v=0, count_p=0, i;
    ...
    for (int i=0;i<gidx_u.size();i++) Iv[i] = gidx_u[i];
    ...
    for (int i=0;i<gidx_u.size();i++) blk_b_FS.val[i] = b_FS.val[gidx_u[i]];
    for (int i=0;i<gidx_p.size();i++) blk_b_FS.val[n_v+i] = b_FS.val[gidx_p[i]];
