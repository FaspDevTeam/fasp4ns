                             ...
                             itsolver_ns_param *itparam)
{
    ...
    switch (itsolver_type) {
        ...
        case SOLVER_BiCGstab:
            ...
            iter=fasp_solver_dblc_pbcgs(A, b, x, prec, tol, MaxIt, stop_type, print_level);
            break;
            
        case SOLVER_MinRes:
            if (print_level>0) printf("Calling MinRes solver (Block CSR format) ...\n");
            iter=fasp_solver_dblc_pminres(A, b, x, prec, tol, MaxIt, stop_type, print_level);
            break;
            
        case SOLVER_GMRES:
            if (print_level>0) printf("Calling GMRES solver (Block CSR format) ...\n");
            iter=fasp_solver_dblc_pvgmres(A, b, x, prec, tol, MaxIt, restart, stop_type, print_level);
            break;
            
        case SOLVER_FGMRES:
            if (print_level>0) printf("Calling FGMRES solver (Block CSR format) ...\n");
            iter=fasp_solver_dblc_pvfgmres(A, b, x, prec, tol, MaxIt, restart, stop_type, print_level);
            break;
            
        case SOLVER_GCR:
            if (print_level>0) printf("Calling GCR solver (Block CSR format) ...\n");
            iter=fasp_solver_dblc_pgcr(A, b, x, MaxIt, tol, prec, print_level, stop_type, restart);
            break;
                                 ...
                                 precond_ns_data *precdata)
{
    ...
    //------ solver phase ------//
    solver_start=clock();
    status=fasp_solver_dblc_itsolver(Mat,b,x,&prec,itparam);
    solver_end=clock();
