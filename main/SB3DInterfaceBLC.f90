SUBROUTINE SB3DInterfaceBLC(aij, rhs, Dim_unknown_P,Dim_unknown_u,Dim_unknown_v,Dim_unknown_w,totalnnz,iglobal,jglobal)
...
!   Arguments
    ...
    INTEGER, INTENT(IN)          :: Dim_unknown_P,Dim_unknown_u,Dim_unknown_v,Dim_unknown_w,totalnnz
    INTEGER, INTENT(IN)          :: iglobal(Dim_unknown_P+Dim_unknown_u+Dim_unknown_v+Dim_unknown_w),jglobal(totalnnz)
    REAL(DBL), INTENT(IN)        :: aij(totalnnz)
    REAL(DBL), INTENT(INOUT)     :: rhs(Dim_unknown_P+Dim_unknown_u+Dim_unknown_v+Dim_unknown_w)
!   Solver Parameter
...
!   CSR
 ...
 !  connect to fasp variables
    ...
    nA = Dim_unknown_u+Dim_unknown_v + Dim_unknown_w
    nB = Dim_unknown_u+Dim_unknown_v + Dim_unknown_w
    nC = Dim_unknown_P
    allocate(ia(nA + 1))
    allocate(ib(nB + 1))
    allocate(ic(nC + 1))
...
!----------------------------------------------------------
    ...
    allocate(iglobal_up(Dim_unknown_u+Dim_unknown_v+Dim_unknown_w))
    allocate(iglobal_uu(Dim_unknown_u+Dim_unknown_v+Dim_unknown_w))
! check the nnz numbers of each matrix
    ...
    nnz_up = 0
    nnz_uu = 0
    nrow = Dim_unknown_u+Dim_unknown_v+Dim_unknown_w
...
END SUBROUTINE SB3DInterfaceBLC
